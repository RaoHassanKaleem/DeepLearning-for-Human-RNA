...
def createModel():
    model = Sequential()
    model.add(Embedding(max_fatures, embed_dim,input_length = fe.X.shape[1]))
...
if st.button('Example'):
    ...
    st.code(r">Seq1\UGCCGAGACCUCAAGGCCAAGGGAAUUUUAUUUGUGGGGAG", language="markdown")
    st.code(r">Seq2\GGCCUGAAAAACAUGAUCAAGGAGGUGGAUGAGGACUUUGA", language="markdown")
    st.info("Negative Sequences")
    st.code(r">Seq1\AAACUCCUUCAUCCAAGUCUGGUUCUUCUCCUCUUGGUCCU", language="markdown")
    st.code(r">Seq2\AAAUCAGGUGGAAGGACAGAGCACCCUUUCACCGUGGAGGA", language="markdown")
...
if st.sidebar.button("SUBMIT"):
    ...
    for rec in records:
        ...
        if(seqValidator(seq)):
            ...
            st.info("Sequence with Sequence ID: " + str(seq_id) + " is invalid, containing letters other than T.")
    fasta_io.close()
